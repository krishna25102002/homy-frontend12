...
const RegisterForm = () => {
    ...
    const registrationSchema = yup.object().shape({
        name: yup.string().when('isOtpStep', {
            is: (isOtpStep: boolean) => isOtpStep,
            then: (schema) => schema.notRequired(),
            otherwise: (schema) => schema.required().label("Name"),
        }),
        email: yup.string().when('isOtpStep', {
            is: (isOtpStep: boolean) => isOtpStep,
            then: (schema) => schema.notRequired(),
            otherwise: (schema) => schema.required().email().label("Email"),
        }),
        mobile: yup.string().when('isOtpStep', {
            is: (isOtpStep: boolean) => isOtpStep,
            then: (schema) => schema.notRequired(),
            otherwise: (schema) => schema.required().label("Mobile"),
        }),
        password: yup.string().when('isOtpStep', {
            is: (isOtpStep: boolean) => isOtpStep,
            then: (schema) => schema.notRequired(),
            otherwise: (schema) => schema.required().min(6).label("Password"),
        }),
        otp: yup.string().when('isOtpStep', {
            is: (isOtpStep: boolean) => !isOtpStep,
            then: (schema) => schema.notRequired(),
            otherwise: (schema) => schema.required().label("OTP"),
        }),
        isOtpStep: yup.boolean().notRequired(),
    });

    type SchemaType = yup.InferType<typeof registrationSchema>;
    ...
    const registerUser = async (data: SchemaType) => {
        try {
